...
def Calculate_uncertainty(feats, uncertainty_type, lamb1, lamb2):
    ######### Calculate alpha
    logits = feats
...
class ResNet_SDD(nn.Module):
    ...
    def forward(self, x, uncertainty_type='max_modified_prob', lamb1=1.0, lamb2=0.1, score_norm=False):
        ...
        patch_score_uc = patch_score.permute((2, 1, 0))
        scores = []
        for i in range(patch_score_uc.shape[-1]):
            score =  Calculate_uncertainty(patch_score_uc[:,:,i].squeeze(), uncertainty_type = uncertainty_type, lamb1=lamb1, lamb2=lamb2)
            scores.append(score)
        scores = [item for sublist in scores for item in sublist]

        return out, patch_score, scores


def resnet18_sdd_redl(pretrained=False, num_classes=None, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
...
def resnet34_sdd_redl(pretrained=False, num_classes=None, **kwargs):
    """Constructs a ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
...
def resnet50_sdd_redl(pretrained=False, num_classes=None, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
...
def resnet101_sdd_redl(pretrained=False, num_classes=None, **kwargs):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
...
def resnet152_sdd_redl(pretrained=False, num_classes=None, **kwargs):
    """Constructs a ResNet-152 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """

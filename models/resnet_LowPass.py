...
def resnet18_LP(pretrained=False, num_classes=None, **kwargs):
    ...
    # 加载预训练权重（跳过stu_low_module_1）
    if pretrained:
        # pretrained_model = models.resnet18(pretrained=True)  # 加载原始的预训练模型
...
def resnet34_LP(pretrained=False, num_classes=None, **kwargs):
    ...
    if pretrained:
        ...
        pretrained_model.load_state_dict(model_zoo.load_url(model_urls['resnet34']))
        pretrained_dict = pretrained_model.state_dict()  # 获取预训练权重字典
        model_dict = model.state_dict()  # 获取当前模型的权重字典
...
def resnet50_LP(pretrained=False, num_classes=None, **kwargs):
    ...
    if pretrained:
        ...
        pretrained_model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
        pretrained_dict = pretrained_model.state_dict()  # 获取预训练权重字典
        model_dict = model.state_dict()  # 获取当前模型的权重字典
...
def resnet101_LP(pretrained=False, num_classes=None, **kwargs):
    ...
    if pretrained:
        pretrained_model = models.resnet101(pretrained=False)  # 加载原始的预训练模型
        pretrained_model.load_state_dict(model_zoo.load_url(model_urls['resnet101']))
        pretrained_dict = pretrained_model.state_dict()  # 获取预训练权重字典
        model_dict = model.state_dict()  # 获取当前模型的权重字典
...
def resnet152_LP(pretrained=False, num_classes=None, **kwargs):
    ...
    if pretrained:
        pretrained_model = models.resnet152(pretrained=False)  # 加载原始的预训练模型
        pretrained_model.load_state_dict(model_zoo.load_url(model_urls['resnet152']))
        pretrained_dict = pretrained_model.state_dict()  # 获取预训练权重字典
        model_dict = model.state_dict()  # 获取当前模型的权重字典
...
# model = resnet18_LP(pretrained=True, num_classes=5)
